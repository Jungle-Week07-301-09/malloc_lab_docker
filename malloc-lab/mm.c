...
static void removeBlock(void *bp,size_t level);
static void putFreeBlock(void *bp,size_t level);
static int level_for_size(size_t size);
static unsigned int int_log2(unsigned int k);
int mm_init(void)
{   
...
static void split_buddy(void *bp, size_t asize)
{
    ...
    while ((size / 2) >= asize && (size / 2) >= 2 * DSIZE)
    {
        ...
        // free list에 삽입
        int log_size = int_log2(size);
        putFreeBlock(buddy, log_size);
    }
}
...
static void* merge_buddy(void* bp){
    ...
    size_t size = GET_SIZE(HDRP(bp));
    size_t log_size = int_log2(size);
    if(size == MAX_LEVEL){
        putFreeBlock(bp, log_size);
        return bp;
    }
...
static void _merge_buddy(void* bp, void* buddy){
    ...
    size_t log_size = int_log2(size);
    removeBlock(bp, log_size);
    removeBlock(buddy, log_size);
    PUT(HDRP(bp),PACK(size, 0));
    PUT(FTRP(buddy), PACK(size, 0));
}
...
static void *find_fit(size_t asize)
{
    ...
    void *bp = HEAD(level_for_size(asize));
    while (bp != NULL) {
        size_t bsize = GET_SIZE(HDRP(bp));
        if (bsize == asize) {
            return bp; // 딱 맞는 블록
        ...
        if (bsize > asize) {
            ...
            // 다시 탐색 (버디가 만들어졌으므로 find_fit 재귀적으로 호출)
...
static void place(void *bp, size_t asize)
{
    ...
    size_t log_size = int_log2(csize);
    removeBlock(bp, log_size);
...
void *mm_malloc(size_t size)
{
    ...
    size_t needed = size + 8;  

    // 최소 블록 또는 2의 거듭제곱으로 올림
    size_t asize = next_power_of_two(needed);

    // 찾고–없으면 확장–배치 과정은 기존 find_fit/place 로직 사용
    ...
    // 힙 확장도 최소 CHUNKSIZE 단위로, 확장 크기도 2의 거듭제곱으로 맞춰주세요
    size_t extendsize = next_power_of_two(MAX(asize, CHUNKSIZE));
    if ((bp = extend_heap(extendsize)) == NULL)
        return NULL;
    place(bp, asize);
    return bp;
